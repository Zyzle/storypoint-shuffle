...
const meta = {
  ...
  async afterEach(context) {
    ...
    await context.userEvent.click(context.canvas.getByTitle("fibonacci"));
  },
...
export const CreateRoomForm: Story = {
  ...
  play: async ({ args, canvas, userEvent, step }) => {
    await step("Click disabled by default", async () => {
      await expect(canvas.getByRole("button", { name: "Create Room" })).toBeDisabled();
    });
    await step("Fill in the form", async () => {
      ...
      await expect(args.onCreate).toHaveBeenCalledWith("Bob", false, "fibonacci");
    });
